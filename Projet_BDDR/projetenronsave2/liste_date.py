...
def Listedate(path=osp.join(os.getcwd(),"maildir"),i=[0],Liste=[],pa=True):
    for file in os.listdir(path) : 
        if osp.isfile(osp.join(path,file)) : #C'est ci_dessous que l'on collecte les données du mail
            ...
            Liste.append(mail.date)
            i[0]+=1
            print(f"({round(100*i[0]/517401,2)}{r'%'}) ajout de la date du fichier {mail.path}")
            print(i)
        else : Listedate(osp.join(path,file), i, Liste, False)
    if pa : return Liste
